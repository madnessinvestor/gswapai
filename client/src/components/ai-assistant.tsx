...
interface AISwapAssistantProps {
  onSwapAction: (from: any, to: any, amount: string) => Promise<void>;
  tokens: any[];
  balances: { USDC: string; EURC: string };
  walletConnected: boolean;
  provider: "groq" | "gemini";
  onProviderChange: (provider: "groq" | "gemini") => void;
}

export default function AISwapAssistant({ onSwapAction, tokens, balances, walletConnected, provider, onProviderChange }: AISwapAssistantProps) {
  ...
  return (
    <div className="flex flex-col h-[400px]">
      ...
      <div className="p-4 border-t border-primary/10 bg-[#1c1038]/50">
        ...
        {walletConnected && (
          <div className="bg-[#130b29]/60 rounded-lg py-2 px-4 flex items-center justify-center gap-6 border border-primary/5 shadow-inner">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.6)] animate-pulse" />
              <span className="text-[13px] font-black text-foreground tracking-wider uppercase font-mono">
                USDC: {balances.USDC}
              </span>
            </div>
            <div className="h-4 w-[1px] bg-primary/20" />
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-yellow-400 shadow-[0_0_8px_rgba(250,204,21,0.6)] animate-pulse" />
              <span className="text-[13px] font-black text-foreground tracking-wider uppercase font-mono">
                EURC: {balances.EURC}
              </span>
            </div>
